...
def pytest_runtest_makereport(item, call):
    ...
    if report.when == "call" and report.failed:
        ...
        <pre><code class="python"># conftest.py - Version limpia y completa
import pytest
...
pytest --html=reports/report_$(date +%Y%m%d_%H%M%S).html --self-contained-html</code></pre>
        ...
        <pre><code class="python"># conftest.py - Nombre de reporte dinamico con timestamp
import pytest
from datetime import datetime
...
# addopts = "--html=reports/report.html --self-contained-html -v"</code></pre>
        ...
        <pre><code class="python"># conftest.py - Configuracion completa de reportes con pytest-html
import pytest
import base64
import platform
import os
...
def pytest_runtest_makereport(item, call):
    ...
    if report.when == "call":
        ...
        <pre><code class="python"># tests/test_login.py - Test de ejemplo que genera reporte completo
...
playwright install chromium</code></pre>
            ...
            <pre><code class="python"># tests/test_ejercicio.py
from playwright.sync_api import Page, expect
